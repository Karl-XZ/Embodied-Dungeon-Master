...
class GameActivity : AppCompatActivity() {
    ...
    // 图片弹窗
    private var imageDialog: com.xmov.metahuman.app.utils.ImageDialog? = null
    ...
    private fun initUI() {
        ...
        // 初始化图片弹窗
        imageDialog = com.xmov.metahuman.app.utils.ImageDialog(this)
    ...
    private fun updateScene() {
        // 主线程保护：如果不在主线程，切回主线程执行
        if (android.os.Looper.myLooper() != android.os.Looper.getMainLooper()) {
            mainHandler.post { updateScene() }
            return
        }
    ...
    private fun updateUI(state: GameState) {
        // 主线程保护：如果不在主线程，切回主线程执行
        if (android.os.Looper.myLooper() != android.os.Looper.getMainLooper()) {
            mainHandler.post { updateUI(state) }
            return
        }
                ...
                if (actionResult.cluesReceived.isNotEmpty()) {
                    ...
                    if (AppSettings.isImageGenEnabled()) {
                        actionResult.cluesReceived.forEach { clue ->
                            lifecycleScope.launch {
                                val imageUrl = featureIntegrationManager.generateClueImage(clue, gameEngine?.gameType ?: GameType.PAOTUAN)
                                imageUrl?.let {
                                    // 可以显示在在线解答中
                                }
                            }
    ...
    private fun showInteractionCheckResult(result: com.xmov.metahuman.app.interaction.CheckResult) {
        ...
        lifecycleScope.launch {
            ...
            // 使用 LLM 格式化结果
            val formattedResult = engine.formatInteractionCheckResult(
    ...
    private fun handleImageGenerationRequest(request: ImageGenerationRequest, engine: com.xmov.metahuman.app.trpg.GameEngine) {
        ...
        lifecycleScope.launch {
            ...
            val finalPrompt = processedPromptResult.getOrNull() ?: request.description

            // 使用处理后的 Prompt 生成图片
            val imageResult = featureIntegrationManager.generateCustomImage(finalPrompt, engine.gameType)

            imageResult.fold(
                onSuccess = { url ->
                    url?.let {
                        // 使用 LLM 格式化结果
                        val formattedResult = engine.formatImageGenerationResult(url, request.fullInput).getOrNull()
                            ?: "图片已生成。"

                        runOnUiThread {
                            // 只使用图片弹窗显示，不显示在右上角
                            imageDialog?.show(url)
                ...
                onFailure = { exception ->
                    val error = "图片生成失败: ${exception.message ?: "未知错误"}"
                    if (AppSettings.isDebugModeEnabled()) {
                        ErrorDialog.showError(
                            this@GameActivity,
                            title = "图片生成失败",
    ...
    private fun showPhotoRecognitionDialog(request: PhotoRecognitionRequest) {
        ...
        lifecycleScope.launch {
            ...
            val result = featureIntegrationManager.captureAndRecognize(request.target)
    ...
    private fun showPhotoRecognitionResult(result: PhotoRecognitionResult?) {
        ...
        lifecycleScope.launch {
            ...
            val formattedResult = engine.formatPhotoRecognitionResult(
